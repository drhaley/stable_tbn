...
class SolverFormulation(Enum):
    STABLE_GEN_FORMULATION = auto()  # TODO: NYI
    BOND_OBLIVIOUS_FORMULATION = auto()  # TODO: NYI
...
class Solver:
    ...
    def stable_configs(self,
        ...
        if formulation == SolverFormulation.BEYOND_MULTISET_FORMULATION:
            ...
            return self.configs_with_number_of_merges(tbn, formulation=formulation, max_number_of_polymers=number_of_polymers)
    ...
    def configs_with_number_of_polymers(self,
        ...
        model, ordered_monomer_types, polymer_composition_vars = self.__build_model(
            tbn, formulation=formulation, max_number_of_polymers=number_of_polymers
        )
    ...
    def configs_with_number_of_merges(self,
        ...
        model, ordered_monomer_types, polymer_composition_vars = self.__build_model(
            ...
            max_number_of_polymers=max_number_of_polymers,
        )
        solutions = self.__adapter.solve_all(model, list(polymer_composition_vars.values()))
        return [
            self.interpret_solution(tbn, ordered_monomer_types, polymer_composition_vars, solution)
    ...
    def __build_model(self,
        ...
        if formulation == SolverFormulation.BEYOND_MULTISET_FORMULATION:
            ...
            if max_number_of_polymers is None:
                optimize = True
            else:
                optimize = False
        ...
        if max_number_of_polymers is None:
            if formulation == SolverFormulation.BEYOND_MULTISET_FORMULATION:
                max_number_of_polymers = total_number_of_limiting_monomers
            else:
                max_number_of_polymers = total_number_of_monomers
        ...
        for i, monomer_type in enumerate(ordered_monomer_types):
            for j in range(max_number_of_polymers):
                polymer_composition_vars[i, j] = model.int_var(
                    0,
                    min(tbn.count(monomer_type), upper_bound_on_total_monomers_in_complexes),  # latter used if count is inf
        ...
        number_of_polymers = sum(indicator_vars[j] for j in range(max_number_of_polymers))
        ...
        if formulation == SolverFormulation.BEYOND_MULTISET_FORMULATION:
            ...
            if optimize:
                ...
                model.Add(number_of_polymers == max_number_of_polymers)
        ...
        if self.__sorted_polymers:
            ...
            for i in range(-1, len(ordered_monomer_types)):
                for j in range(max_number_of_polymers - 1):
