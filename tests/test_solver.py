...
class TestSolver(unittest.TestCase):
    ...
    def __run_test_stable_config_with_specific_solver(self, solver: Solver, parameter_string: str):
        ...
        with self.subTest("check for no combinatorial duplicates", parameter_string=parameter_string):
            test_tbn = Tbn.from_string("2[a* b*] \n a b")
            config = solver.stable_config(test_tbn)
            self.assertEqual(2, config.number_of_polymers())
            self.assertEqual(1, config.number_of_merges())

        with self.subTest("check for return of multiple stable configurations", parameter_string=parameter_string):
    ...
    def __run_test_configs_with_number_of_polymers_with_specific_solver(self, solver: Solver, parameter_string: str):
        with self.subTest("check for correctness in simple example", parameter_string=parameter_string):
            test_tbn = Tbn.from_string("a* b* \n a b \n a* \n b*")
            results = {
                i: list(solver.configs_with_number_of_polymers(test_tbn, number_of_polymers=i))
                    for i in range(1, 5)
            }
